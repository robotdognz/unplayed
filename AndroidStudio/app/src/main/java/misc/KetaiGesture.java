 ...
 * gesture events a sketch can define the following methods:<br />
 * <br />
 * 
 * void onTap(float x, float y) - x, y location of the tap<br />
 * void onDoubleTap(float x, float y) - x,y location of double tap<br />
 * void onFlick(float x, float y, float px, float py, float v) - x,y where flick
 * ended, px,py - where flick began, v - velocity of flick in pixels/sec <br />
 * void onScroll(int x int y) - not currently used<br />
 * void onLongPress(float x, float y) - x, y position of long press<br />
 * void onPinch(float x, float y, float r) - x,y of center, r is the distance
 * change<br />
 * void onRotate(float x, float y, float a) - x, y of center, a is the angle
 * change in radians<br />
 */
public class KetaiGesture implements OnGestureListener, OnDoubleTapListener {
	...
	Method onDoubleTapMethod, onScrollMethod, onFlickMethod, onTapMethod, onLongPressMethod, onPinchMethod,
			onRotateMethod;
		...
		if (onFlickMethod != null) {
			try {
				...
				onFlickMethod.invoke(parent,
						new Object[] { arg1.getX(), arg1.getY(), arg0.getX(), arg0.getY(), v.mag() });
			} catch (Exception e) {
			}
		}
		return true;
	...
	public void onLongPress(MotionEvent arg0) {
		if (onLongPressMethod != null) {
			try {
				onLongPressMethod.invoke(parent, new Object[] { arg0.getX(), arg0.getY() });
			} catch (Exception e) {
			}
		}
	}
	...
	public boolean onSingleTapUp(MotionEvent arg0) {
		if (onTapMethod != null) {
			try {
				onTapMethod.invoke(parent, new Object[] { arg0.getX(), arg0.getY() });
			} catch (Exception e) {
			}
		}
		return true;
	...
	public boolean onDoubleTap(MotionEvent arg0) {
		if (onDoubleTapMethod != null) {
			try {
				onDoubleTapMethod.invoke(parent, new Object[] { arg0.getX(), arg0.getY() });
			} catch (Exception e) {
			}
		}
		return true;
	...
	private void findParentIntentions() {
		...
		try {
			onTapMethod = parent.getClass().getMethod("onTap", new Class[] { float.class, float.class });
		} catch (Exception e) {
		}

		try {
			onDoubleTapMethod = parent.getClass().getMethod("onDoubleTap", new Class[] { float.class, float.class });
		} catch (Exception e) {
		}

		try {
			onFlickMethod = parent.getClass().getMethod("onFlick",
					new Class[] { float.class, float.class, float.class, float.class, float.class });
		} catch (Exception e) {
		}

		try {
			onScrollMethod = parent.getClass().getMethod("onScroll", new Class[] { int.class, int.class });
		} catch (Exception e) {
		}

		try {
			onLongPressMethod = parent.getClass().getMethod("onLongPress", new Class[] { float.class, float.class });
		} catch (Exception e) {
		}
	...
	private synchronized void analyse() {
		if (cursors.size() > 1 && pcursors.size() > 1) {
			...
			float oldangle = PApplet.atan2(PVector.sub(p1, p2).y, PVector.sub(p1, p2).x);
			float newangle = PApplet.atan2(PVector.sub(c1, c2).y, PVector.sub(c1, c2).x);

			float delta = (newangle - oldangle);

			if (onPinchMethod != null) {
				try {
					onPinchMethod.invoke(parent, new Object[] { midx, midy, dc - dp });
				} catch (Exception e) {
				}
			}
			...
			if (onRotateMethod != null) {
				try {
					onRotateMethod.invoke(parent, new Object[] { midx, midy, delta });
				} catch (Exception e) {
				}
			}
		}
