...
public class BufferUtils {
	 ...
	 * NULL. If 'userSuppliedCapacity' is not zero, buffer is user supplied and must
	 * be kept.
	 */
	public static <T> T[] reallocateBuffer(Class<T> klass, T[] buffer, int userSuppliedCapacity, int oldCapacity,
	...
	/**
	 * Reallocate an int buffer. A 'deferred' buffer is reallocated only if it is
	 * not NULL. If 'userSuppliedCapacity' is not zero, buffer is user supplied and
	 * must be kept.
	 */
	public static int[] reallocateBuffer(int[] buffer, int userSuppliedCapacity, int oldCapacity, int newCapacity,
			boolean deferred) {
		assert (newCapacity > oldCapacity);
		assert (userSuppliedCapacity == 0 || newCapacity <= userSuppliedCapacity);
		if ((!deferred || buffer != null) && userSuppliedCapacity == 0) {
	...
	/**
	 * Reallocate a float buffer. A 'deferred' buffer is reallocated only if it is
	 * not NULL. If 'userSuppliedCapacity' is not zero, buffer is user supplied and
	 * must be kept.
	 */
	public static float[] reallocateBuffer(float[] buffer, int userSuppliedCapacity, int oldCapacity, int newCapacity,
			boolean deferred) {
		assert (newCapacity > oldCapacity);
		assert (userSuppliedCapacity == 0 || newCapacity <= userSuppliedCapacity);
		if ((!deferred || buffer != null) && userSuppliedCapacity == 0) {
