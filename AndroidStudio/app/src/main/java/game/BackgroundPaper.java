...
public class BackgroundPaper {
	...
	public void draw(PGraphics graphics, PVector topLeft, PVector bottomRight, float scale) {
		...
		float startX = (int) Math.round((topLeft.x - (gridSize / 2f)) / gridSize);
		// find y start position;
		float startY = (int) Math.round((topLeft.y - (gridSize / 2f)) / gridSize);
		// find x end position
		float endX = (int) Math.round((bottomRight.x + (gridSize / 2f)) / gridSize);
		// find y end position
		float endY = (int) Math.round((bottomRight.y + (gridSize / 2f)) / gridSize);
	...
	public void drawShape(PApplet p, PVector topLeft, PVector bottomRight, float scale) {
		...
		paper.texture(TextureCache.getPageViewBackground(scale));
		paper.vertex(startX, startY, xTileStart, yTileStart); // top left
		paper.vertex(endX, startY, xTileEnd, yTileStart); // top right
		paper.vertex(endX, endY, xTileEnd, yTileEnd); // bottom right
		paper.vertex(startX, endY, xTileStart, yTileEnd); // bottom left
